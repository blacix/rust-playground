...
use futures::{pin_mut, stream::SelectAll, StreamExt, executor::block_on};
...
pub async fn ble_main() -> bluer::Result<()> {
    ...
    let mut x = adapter.is_discovering().await;
    if let Ok(true) = x {
        println!("!!!!!!!!already discovering");
    } else {
        println!("!!!!!!!!not discovering")
    }
    println!("!!!!!!!!!!STUFFF");
    // match x {
    //     Ok(true)  => print!("already discovering"),
    //     Ok(false) => print!("not discovering"),
    //     _ => print!("not discovering"),
    // }
    let device_events = adapter.discover_devices().await?;
    pin_mut!(device_events);

    x = adapter.is_discovering().await;
    if let Ok(true) = x {
        println!("2!!!!!!!!already discovering");
    } else {
        println!("2!!!!!!!!not discovering")
    }
